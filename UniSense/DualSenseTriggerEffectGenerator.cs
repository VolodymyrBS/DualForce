...
namespace DualForce
{
    /// Base of https://gist.github.com/Nielk1/6d54cc2c00d2201ccb8c2720ad7538db
    /// <remarks>
    ...
    /// </summary>
    public static class DualSenseTriggerEffectGenerator
    {
        ...
        /// </summary>
        /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
        ...
        public static bool Off(Span<byte> destinationArray, int destinationIndex)
        {
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool Feedback(Span<byte> destinationArray, int destinationIndex, byte position, byte strength)
        {
            if (position > 9)
                return false;
            if (strength > 8)
        ...
        /// <param name="strength">The force of the resistance. Must be between 0 and 8 inclusive.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool Weapon(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte strength)
        {
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool Vibration(Span<byte> destinationArray, int destinationIndex, byte position, byte amplitude, byte frequency)
        {
            if (position > 9)
                return false;
            if (amplitude > 8)
        ...
        /// This is an offical effect and is expected to be present in future DualSense firmware.
        /// </summary>
        /// <seealso cref="Feedback(Span&lt;byte&gt;, int, byte, byte)"/>
        /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
        /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
        /// <param name="strength">Array of 10 resistance values for zones 0 through 9. Must be between 0 and 8 inclusive.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool MultiplePositionFeedback(Span<byte> destinationArray, int destinationIndex, Span<byte> strength)
        {
            if (strength.Length != 10) return false;

            if (AnyGreaterZero(strength))
            {
                UInt32 forceZones  = 0;
                UInt16 activeZones = 0;
                for (int i = 0; i < 10; i++)
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool SlopeFeedback(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte startStrength, byte endStrength)
        {
            ...
            Span<byte> strength = stackalloc byte[10];
        ...
        /// </remarks>
        /// <seealso cref="Vibration(Span&lt;byte&gt;, int, byte, byte, byte)"/>
        /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
        /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
        /// <param name="amplitude">Array of 10 strength values for zones 0 through 9. Must be between 0 and 8 inclusive.</param>
        /// <param name="frequency">Frequency of the automatic cycling action in hertz.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool MultiplePositionVibration(Span<byte> destinationArray, int destinationIndex, byte frequency, Span<byte> amplitude)
        {
            if (amplitude.Length != 10) return false;

            if (frequency > 0 && AnyGreaterZero(amplitude))
            {
        ...
        private static bool AnyGreaterZero(Span<byte> data)
        {
        ...
        /// <summary>
        /// The effect resembles the <see cref="Weapon(Span&lt;byte&gt;, int, byte, byte, byte)">Weapon</see>
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool Bow(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte strength, byte snapForce)
        {
            if (startPosition > 8)
                return false;
            if (endPosition > 8)
        ...
        /// <param name="frequency">Frequency of the automatic cycling action in hertz.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool Galloping(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte firstFoot, byte secondFoot, byte frequency)
        {
            if (startPosition > 8)
                return false;
            if (endPosition > 9)
        ...
        /// <summary>
        /// This effect resembles <see cref="Vibration(Span&lt;byte&gt;, int, byte, byte, byte)">Vibration</see>
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool Machine(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte amplitudeA, byte amplitudeB, byte frequency, byte period)
        {
            if (startPosition > 8)
                return false;
            if (endPosition > 9)
        ...
        /// </summary>
        /// <remarks>
        /// Use <see cref="Feedback(Span&lt;byte&gt;, int, byte, byte)"/> instead.
        /// </remarks>
        /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
        /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool Simple_Feedback(Span<byte> destinationArray, int destinationIndex, byte position, byte strength)
        {
        ...
        /// </summary>
        /// <remarks>
        /// Use <see cref="Weapon(Span&lt;byte&gt;, int, byte, byte, byte)"/> instead.
        /// </remarks>
        /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
        /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
        ...
        /// <param name="strength">The force of the resistance.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool Simple_Weapon(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte strength)
        {
        ...
        /// </summary>
        /// <remarks>
        /// Use <see cref="Vibration(Span&lt;byte&gt;, int, byte, byte, byte)"/> instead.
        /// </remarks>
        /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
        /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
        /// <param name="position">The starting zone of the trigger effect.</param>
        ...
        /// <param name="frequency">Frequency of the automatic cycling action in hertz.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool Simple_Vibration(Span<byte> destinationArray, int destinationIndex, byte position, byte amplitude, byte frequency)
        {
        ...
        /// <returns>The success of the effect write.</returns>
        public static bool Limited_Feedback(Span<byte> destinationArray, int destinationIndex, byte position, byte strength)
        {
            if (strength > 10)
                return false;
            if (strength > 0)
        ...
        /// <param name="strength">The force of the resistance. Must be between 0 and 10 inclusive.</param>
        /// <returns>The success of the effect write.</returns>
        public static bool Limited_Weapon(Span<byte> destinationArray, int destinationIndex, byte startPosition, byte endPosition, byte strength)
        {
        ...
        public static class Apple
        {
            ...
            /// </remarks>
            /// <seealso cref="Off(Span&lt;byte&gt;, int)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool SetModeOff(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <returns>The success of the effect write.</returns>
            public static bool SetModeFeedbackWithStartPosition(Span<byte> destinationArray, int destinationIndex, float startPosition, float resistiveStrength)
            {
            ...
            /// Documentation ported from Apple's API Docs.
            /// </remarks>
            /// <seealso cref="Weapon(Span&lt;byte&gt;, int, byte, byte, byte)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            ...
            /// <param name="resistiveStrength">A normalized float from [0-1], with 0 representing the minimum effect strength (off entirely) and 1 representing the maximum effect strength.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool SetModeWeaponWithStartPosition(Span<byte> destinationArray, int destinationIndex, float startPosition, float endPosition, float resistiveStrength)
            {
                startPosition = (float)Math.Round(startPosition * 9.0f);
            ...
            /// <returns>The success of the effect write.</returns>
            public static bool SetModeVibrationWithStartPosition(Span<byte> destinationArray, int destinationIndex, float startPosition, float amplitude, float frequency)
            {
                startPosition = (float)Math.Round(startPosition * 9.0f);
            ...
            /// Documentation ported from Apple's API Docs.
            /// </remarks>
            /// <seealso cref="MultiplePositionFeedback(Span&lt;byte&gt;, int, Span{T}<byte>)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            ...
            /// <returns>The success of the effect write.</returns>
            public static bool SetModeFeedback(Span<byte> destinationArray, int destinationIndex, float[] positionalResistiveStrengths)
            {
                if (positionalResistiveStrengths.Length != 10) return false;

                Span<byte> force = stackalloc byte[10];
                for (int i = 0; i < 10; i++)
            ...
            /// Documentation ported from Apple's API Docs.
            /// </remarks>
            /// <seealso cref="MultiplePositionFeedback(Span&lt;byte&gt;, int, byte)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <param name="startPosition">A normalized float from [0-1], with 0 representing the smallest possible depression and 1 representing the maximum trigger depression. The effect will begin once the trigger is depressed beyond this point.</param>
            /// <param name="endPosition">A normalized float from [0-1], with 0 representing the smallest possible depression and 1 representing the maximum trigger depression. Must be greater than startPosition. The effect will end once the trigger is depressed beyond this point.</param>
            ...
            /// <returns>The success of the effect write.</returns>
            public static bool setModeSlopeFeedback(Span<byte> destinationArray, int destinationIndex, float startPosition, float endPosition, float startStrength, float endStrength)
            {
                startPosition = (float)Math.Round(startPosition * 9.0f);
                endPosition = (float)Math.Round(endPosition * 9.0f);
            ...
            /// Documentation ported from Apple's API Docs.
            /// </remarks>
            /// <seealso cref="MultiplePositionVibration(Span&lt;byte&gt;, int, byte, Span{T}<byte>)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            ...
            /// <param name="frequency">A normalized float from [0-1], with 0 representing the minimum frequency and 1 representing the maximum frequency of the vibration effect.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool setModeVibration(Span<byte> destinationArray, int destinationIndex, float[] positionalAmplitudes, float frequency)
            {
        ...
        public static class ReWASD
        {
            ...
            /// </remarks>
            /// <seealso cref="Simple_Weapon(Span&lt;byte&gt;, int, byte, byte, byte)"/>
            /// <seealso cref="Weapon(Span&lt;byte&gt;, int, byte, byte, byte)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool FullPress(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool SoftPress(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool MediumPress(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool HardPress(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool Pulse(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool Choppy(Span<byte> destinationArray, int destinationIndex)
            {
                destinationArray[destinationIndex + 0] = (byte)TriggerEffectType.Feedback;
            ...
            /// </remarks>
            /// <seealso cref="Simple_Feedback(Span&lt;byte&gt;, int, byte, byte)"/>
            /// <seealso cref="Feedback(Span&lt;byte&gt;, int, byte, byte)"/>
            /// <param name="destinationArray">The Span&lt;byte&gt; that receives the data.</param>
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool SoftRigidity(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool MediumRigidity(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool MaxRigidity(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <param name="destinationIndex">A 32-bit integer that represents the index in the destinationArray at which storing begins.</param>
            /// <returns>The success of the effect write.</returns>
            public static bool HalfPress(Span<byte> destinationArray, int destinationIndex) =>
            ...
            /// <returns>The success of the effect write.</returns>
            public static bool Rifle(Span<byte> destinationArray, int destinationIndex, byte frequency = 10)
            {
                if (frequency < 2)
                    return false;
                if (frequency > 20)
            ...
            /// <returns>The success of the effect write.</returns>
            public static bool Vibration(Span<byte> destinationArray, int destinationIndex, byte strength = 220, byte frequency = 30)
            {
                if (strength < 1)
                    return false;
                if (frequency < 1)
